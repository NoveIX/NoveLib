...
function Convert-MaskToCIDR {
    ...
    if ($Mask -notmatch '^(\d{1,3}\.){3}\d{1,3}$') {
        ...
        $sysThrMsg = "Invalid IPv4 subnet mask format: '$Mask'."
        throw [System.FormatException]::new($sysThrMsg)
    }
    ...
    $binary = ($Mask.Split('.') | ForEach-Object {
            if ($_ -gt 255 -or $_ -lt 0) {
                ...
                $sysThrMsg = "Octet '$_' is out of range (0-255)."
                throw [System.ArgumentOutOfRangeException]::new($sysThrMsg)
            }
    ...
    if ($binary -notmatch '^1*0*$') {
        ...
        $sysThrMsg = "Subnet mask is not valid: bits are not contiguous (binary: $binary)."
